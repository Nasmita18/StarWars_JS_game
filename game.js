...
var pipeUp = new Image(); // Объект сосулька сверху
...
pipeUp.src = "img/pipeUp.png"; // Изображение сосулька сверху
...
function draw() { 
    ...
    for (var i = 0; i < pipe.length; i++) { // Цикл отрисовки сосулек
        // ctx.drawImage(pipeUp, pipe[i].x, pipe[i].y); // Отрисовка верхней сосульки
        let num = 10
        ctx.drawImage(sosulka, pipe[i].x, pipe[i].y + pipeUp.height + gap); // Отрисовка нижней сосульки
        ...
        if (xPos + starship.width >= pipe[i].x && xPos <= pipe[i].x + pipeUp.width
            &&
            (yPos + starship.height >= pipe[i].y + pipeUp.height + gap)
